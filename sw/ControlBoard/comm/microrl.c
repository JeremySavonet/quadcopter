...
#ifdef _USE_CTLR_C
//*****************************************************************************
void microrl_set_sigint_callback (microrl_t * this, void (*sigintf)(struct microrl_t *))
{
    this->sigint = sigintf;
}
#endif
...
void new_line_handler(microrl_t * this){
    ...
    if (status == -1){
        //          this->print ("ERROR: Max token amount exceeded\n");
...
void mlab_sigint(microrl_t * this)
{
    terminal_newline (this);
    //  this->tkn_arr[0] = "";
    //  this->execute (1, this->tkn_arr);
    print_prompt (this);
    this->cmdlen = 0;
    this->cursor = 0;
    memset(this->cmdline, 0, _COMMAND_LINE_LEN);
...
#endif
            case KEY_FF:
                this->print("\033[2J");    // ESC seq for clear entire screen
                this->print("\033[H");     // ESC seq for move cursor at left-top corner
                print_prompt (this);
                this->print(this->cmdline);
                break;
                //-----------------------------------------------------
            default:
                if ((ch == ' ') && (this->cmdlen == 0))
